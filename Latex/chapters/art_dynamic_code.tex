...
without any additional adaption, \code{mmap()} can map a whole file, returns a pointer
...
are necessary when it shall be executed, written or read later (\code{PROT\_EXEC},
 \code{PROT\_READ}, \code{PROT\_WRITE}). Executing code out of heap is forbidden by 
 default.
...
Like explained in \parencite{jit_intro}, one should go with \code{mmap()} over \code{malloc()} because protection bits can only be set at page boundaries. That's why with
\code{malloc()}, it needs to be ensured manually that the allocation is aligned at a 
page boundary. If it is not the case, a \code{mprotect()} call will have the side effect of disabling/enabling more memory than actually required.

In the end, ``code'' stands for machine code that will get executed, which is not more
...
into memory but then the process needs to be adapted in order to find those executable
parts in memory (linker is responsible for doing that before the program gets executed).
...
shared objects out of a file or an Internet socket right into the process.
A key part of this program is the patching of ``\code{ld-2.19.so}'' that is mapped within the process. Porting that library to Android is not that simple since Android
is not using this library but an own linker at \code{/system/bin/linker}.

So let's try to invoke machine code directly without any file format overhead like
in binaries and shared objects. Again, a simple multiplication function will be implemented to show a proof of concept, shown in \autoref{machineCodeMulC}.
...
Objdump can then show mnemonics as well as machine code
for every function, exactly what will be needed (output for
\code{machineCodeMul.o}'s \code{mul()} function shown in \autoref{tab:mul_objdump}).
\begin{table}[htb]
...
\end{table}
The byte sequence \code{e0000091} represents the mnemonic equivalent of a multiplication
...
to memory have to be inverted first (\code{0xe0000091 -> 0x91 0x00 0x00 0xe0}).

\subsubsection{Implementation}
At first, a memory location will get allocated, a function for that purpose could be written that takes the regions size as an input like shown in \autoref{alloc_exe_mem}.
...
if no file is being specified as a \code{mmap()} parameter (instead a -1),
...
\end{lstlisting}
A \code{typedef} can be used to specify the function signature matching the C source function. It can get initialized by pointing to the allocated memory region. 
After that, all that's left to do is calling it like a common function.
\autoref{exec_mach_code} shows the actual implementation as a JNI C++ function using the methods
from before and showing some additional output information.
...
\end{lstlisting}
Note that \code{func} is just treated as a variable, means its address relies on the stack region (as it is supposed to be) while \code{m}'s address is being displayed in the \code{/proc/self/maps} output without any \code{[libc\_malloc]} annotation or path.
