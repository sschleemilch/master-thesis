...
It has however also a downside since it returns a memory region that is marked 
as read and write but not as executable. So additional steps has to be performed
to use that space for executing code, like calling \code{mprotect()} which needs
the starting address, the size of its region and of course the permissions to set.
