...
\section{Android Virtual Memory Mapping}\label{section:virtual_memory}
...
Let's evaluate which sections are used to store heap as well as common stack
elements by creating a native function with a buffer using \code{malloc()}
and a common local integer variable.
...
fp->_close;
\end{lstlisting}
When initializing a variable in a function and printing out its address afterwards,
one can see that the region is not described in the maps output. Unlike the
common variable, the \code{malloc()} space lies in between the described maps
address range. The region containing the space requested by \code{malloc()} 
is marked as \code{[anon:libc\_malloc]}. That can be proved by reading the mapping
before and after the \code{malloc()} call while only printing lines containing ``\code{[anon:libc\_malloc]}'' and comparing those outcomes. 
Then it appears that one map section differs in address range equal to the 
\code{malloc()} size.  

TODO:mmap() difference on mapping?; why malloc useful/important?; write at a location?