...
and root rights by first copying those files to the \code{/sdcard/}
path and finally using \code{adb pull}.

The decompilation is good in general but naturally more time
...
calls through ``\code{env}'' like ``\code{const char *input = env->GetStringUTFChars(str, NULL);}'' can be interpreted quite good,
