...
Like explained in \parencite{jit_intro}, one should go with \code{mmap()} over \code{malloc()} because protection bits can only be set at memory page boundaries.
That's why with \code{malloc()}, it needs to be ensured manually that the allocation is aligned at a 
...
to memory have to be inverted first (\code{0xe0000091 turns to 0x910000e0}) before
executing them.
...
\end{lstlisting}
A \code{typedef} is being used to specify the function signature matching the C source function that is planned to be injected.
...
This method of executing machine code is quite simple if no external libraries are involved so that the resulting machine code in the object file can be copied directly
...
use the output of \code{/proc/self/maps} triggering for the permission triple 
``\code{rw-p}''. \autoref{gen_mem_write} for instance triggers for the writable
\code{/system/bin/linker} entry. The size of the memory region can be parsed out 
of that matching line. \code{strtoll} can be used to do the actual parsing from a 
hexadecimal char sequence to its \code{long long int} number and can be casted to 
