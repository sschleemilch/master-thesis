...
data. The first one is to map a well known format like a shared object or an executable
into memory but then the process needs to be adapted in order to find those executable
parts in memory (linker is responsible to do that). So one would end up writing linker
functionality or patching the one mapped into the process. It does exist a proof of concept for a Linux x86_64 system shown in \code{memdlopen}. A key part of it is the patching of ``\code{ld-2.19.so}'' in memory.
