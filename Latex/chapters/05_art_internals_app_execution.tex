...
created OAT files for apps with embedded dex and native code with its structure explained in \autoref{chapter:art_internals_app_executable_format}. The invoke method in the \code{Method} class defines a native method as its implementation (see \autoref{method_invoke}).
...
however is quite and forwards the invoke call to the \code{InvokeMethod} implementation in
``\code{reflection.cc}''. 
