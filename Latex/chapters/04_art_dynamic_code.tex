...
at runtime. All that's needed is the \code{loadLibrary()} call. Physically, those libraries
are stored at the same path as the app APK file like introduced earlier. By applying this 
method, those native libs are shipped together within the application that might not be the 
indended behavior.

\section{Native Dynamic Shared Library Loading}\label{section:shared_library_loading}
