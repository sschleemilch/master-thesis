\chapter{Copy Protection and ART}
...
``\code{app\_process}'' call with specific parameters for a Zygote start.
...
Although this example is written for Dalvik since it consists of DVM
...
ToLoad.class}''. The \code{dexPath} has to be readable by the app itself. Therefore
...
with ART, dynamic code loading at Java layer is not that useful since the optimized
file does persist after the first dynamic load and can be analyzed afterwards.
