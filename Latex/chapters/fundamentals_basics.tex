...
Since Apps are sandboxed, they can't access each others data which can be seen as a limitation if Apps should be able to communicate with each other.
...
the \code{lib/} folder contains compiled library directories
...
layouts and menus. The \code{META-INF/} directory holds
the signatures for the specific App, similar to a JAR file.
The heart of every App is the \code{classes.dex} that
...
Kit (JDK) tool ``\code{javac}'' creates a JAR out of every inputted
 \code{.java} file while the Google build tool ``\code{dx}'' takes over
 the transition from \code{.jar} to \code{.dex} ~\parencite{dxtool}.
 An \code{ApkBuilder} takes the DEX file, adding \code{.so} files and
 other resources and finally builds the \code{base.apk} that gets signed
...
The \code{lib/} directory is extracted and gets copied
into that path. \code{classes.dex} is optimized by either ``\code{dexopt}''
or ``\code{dex2oat}'' depending which runtime is present (DVM or ART).
In both cases, the outcoming file is no more machine independent
and is stored at \code{/data/dalvik-cache/<arch>/} with the name
\code{data@app@<packagename>.<appname>-1@base.apk@classes.dex}.
That path however, did change with Android version 6 to
\code{/data/app/oat/<pachagename>.<appname>/<arch>/base.odex}. It
...
It is the same (\code{.dex}) with both runtimes but is actually a totally
...
tool for that conversion step is called ``\code{dex2oat}''. Still, the file
...
file stored at \code{/data/dalvik-cache/...@classes.dex} as described in
